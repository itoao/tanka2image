  ...
  const handleInstagramExport = async () => {
    ...
    try {
      ...
      canvas.toBlob(async (blob) => {
        ...
        // For Instagram Stories, try to use Web Share API (works better on mobile)
        if (navigator.share && navigator.userAgent.match(/iPhone|iPad|Android/i)) {
          try {
            const file = new File([blob], `tanka_story_${Date.now()}.png`, { type: 'image/png' });
            await navigator.share({
              files: [file],
              title: '短歌をInstagram Storiesでシェア',
              text: content
            });
          } catch (shareError) {
            console.log('Share failed:', shareError);
            alert('シェアに失敗しました。ブラウザがシェア機能に対応していない可能性があります。');
          }
        } else {
          // Desktop or browsers without share API - just show message
          alert('モバイルデバイスでこの機能をご利用ください。');
        }
      }, 'image/png');
